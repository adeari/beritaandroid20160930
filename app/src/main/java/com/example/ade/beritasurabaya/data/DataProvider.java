...
public class DataProvider {
    ...
    public static List<BaseItem> getInitialItems() {
        return getSubItems(new GroupItem("root", null, R.drawable.ic_alarm_off_red_24dp));
    }
    ...
    public static List<BaseItem> getSubItems(BaseItem baseItem) {
        ...
        for (int i = 0; i < ITEMS_PER_LEVEL; ++i) {
            ...
            if (i % 2 == 0 && nextLevel != MAX_LEVELS) {
                item = new GroupItem("Group " + Integer.toString(++groupNr), null, R.drawable.ic_alarm_off_red_24dp);
                ((GroupItem) item).setLevel(nextLevel);
            } else {
                item = new Item("Item " + Integer.toString(++itemNr), null, R.drawable.ic_alarm_off_red_24dp);
            }
            result.add(item);
        }
        return result;
